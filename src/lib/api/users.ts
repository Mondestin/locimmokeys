...
export async function updateUserProfile(data: Partial<UserProfile>, photoFile?: File | null) {
  ...
  if (photoFile) {
    try {
      ...
      const photoRef = ref(storage, `profile-photos/${auth.currentUser.uid}/${Date.now()}-${photoFile.name}`);
